      ...
      module Caso
        ...
        included do
          ...
          has_many :anexo_caso,
            ...
            inverse_of: :caso
          accepts_nested_attributes_for :anexo_caso,
            allow_destroy: true,
            reject_if: :all_blank
          has_many :msip_anexo,
            through: :anexo_caso,
          ...
          accepts_nested_attributes_for :caso_etiqueta,
            allow_destroy: true,
            reject_if: :all_blank
          ...
          has_many :caso_fotra,
            ...
            inverse_of: :caso
          accepts_nested_attributes_for :caso_fotra,
            allow_destroy: true,
            reject_if: :all_blank
          ...
          accepts_nested_attributes_for :caso_fuenteprensa,
            allow_destroy: true,
            reject_if: :all_blank
          has_many :fuenteprensa,
            through: :caso_fuenteprensa,
          ...
          accepts_nested_attributes_for :caso_ubicacionpre,
            allow_destroy: true,
            reject_if: :all_blank
          has_many :ubicacionpre,
            through: :caso_ubicacionpre,
          ...
          accepts_nested_attributes_for :victima,
            allow_destroy: true,
            reject_if: :all_blank
          ...
          accepts_nested_attributes_for :combatiente,
            allow_destroy: true,
            reject_if: :all_blank

          has_one :sivel2_gen_conscaso, foreign_key: "caso_id"

          belongs_to :intervalo,
            validate: true,
            class_name: "Sivel2Gen::Intervalo",
            optional: true
          ...
          def fuenteprensa_fecha_y_fuente_unicas
            fuentesprensa = caso_fuenteprensa
            valfp = []
            fuentesprensa.each do |fp|
              if fp && fp.fuenteprensa &&
                ...
                valfp.push([fp.fecha, if fp.fuenteprensa
          ...
          def importa(datosent, datossal, formato_sexo,
            ...
            actos_colectivos = []
            actos = datosent["acto"]
            unless datosent["acto"].is_a?(Array)
              actos = [datosent["acto"]]
            end
            actos.each do |acto|
            ...
            if datosent["grupo"]
              ...
              ldgrupo.each do |pr|
                next unless prs.include?(pr["id_grupo"])
            ...
            if datosent["grupo"]
              if datosent["grupo"].is_a?(Array)
                datosent["grupo"].each do |vc|
                  next unless vicol_acolectivos.include?(vc["id_grupo"])
              ...
              elsif vicol_acolectivos.include?(datosent["grupo"]["id_grupo"])
                victcol = Sivel2Gen::Victimacolectiva.new
                victcol.caso_id = id
                victcol.importa(
            ...
            idsv = {}
            if datosent["victima"]
              if datosent["victima"].is_a?(Array)
            ...
            if datosent["combatiente"]
              if datosent["combatiente"].is_a?(Array)
          ...
          def presenta(atr)
            ...
            when "fuentesfrecuentes"
              caso_fuenteprensa.inject("") do |memo, cf|
                r = (memo == "" ? "" : memo + ".   ")
            ...
            when "presponsables"
              caso_presponsable.inject("") do |memo, pr|
                r = ""
                if pr
                  r = (memo == "" ? "" : memo + ".   ")
                  ...
                  if pr.categoria && pr.categoria.count > 0
                    r += " "
                    r += pr.categoria.inject("") do |memo, c|
                      (memo == "" ? "" : memo + ", ") +
              ...
              victima.inject("") do |memo, v|
                ...
                if v.persona.pais_id
                  r += " - " + Msip::UbicacionHelper.formato_ubicacion_partes(
                    v.persona.pais_id,
                    v.persona.departamento_id,
                    v.persona.municipio_id,
                    v.persona.centropoblado_id,
                    true,
                    true,
                  )
                end
            ...
            when "victimascolectivas"
              victimacolectiva.inject("") do |memo, vc|
                r = (memo == "" ? "" : memo + ".   ")
            ...
            when "actos"
              ...
              col = actocolectivo.inject("") do |memo, a|
                r = (memo == "" ? "" : memo + "; ")
                r += a.presponsable.nombre
                r += " : " + a.categoria.supracategoria.tviolencia_id +
                  a.categoria.id.to_s
            ...
            when "etiquetas"
              caso_etiqueta.inject("") do |memo, ce|
                r = (memo == "" ? "" : memo + ".   ")
