        ...
        included do
          ...
          def sin_identificacion
            if nombres == "N" && apellidos == "N"
              caso = Sivel2Gen::Victima.where(persona_id: id)[0].caso_id
              vics = Sivel2Gen::Victima.where(caso_id: caso)
              hv = {}
              ban = 1
              vics.each_with_index do |vi, _index|
                if vi.persona.nombres == "N" && vi.persona.apellidos == "N"
                  hv[vi.id] = ban
                  ban += 1
                end
              end
              "PERSONA SIN IDENTIFICAR #{hv[Sivel2Gen::Victima.where(persona_id: id)[0].id]}"
            else
