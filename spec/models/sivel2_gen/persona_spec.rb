...
module Sivel2Gen
  RSpec.describe Persona, :type => :model do
    ...
    it "no valido por año errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, anionac: 1)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por mes errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, mesnac: 0)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por mes errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, mesnac: 13)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por dia errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, dianac: 0)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por dia errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, dianac: 32)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por dia errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, mesnac: 4, dianac: 31)
      expect(persona).not_to be_valid
      persona.destroy
    end
    it "no valido por dia errado" do
      persona = FactoryGirl.build(:sivel2_gen_persona, mesnac: 2, dianac: 30)
      expect(persona).not_to be_valid
      persona.destroy
    end
