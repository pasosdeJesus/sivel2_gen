...
xml.relato do
  ...
  if caso.victima
    caso.victima.each do |v|
      xml.persona do
        ...
        xml.observaciones(
          v.persona.etnia.nombre,
          { tipo: "etnia" },
        ) if v.persona.etnia
        xml.observaciones(
          v.persona.pais.nombre,
          { tipo: "pais" },
        ) if v.persona.pais
        xml.observaciones(
          v.persona.departamento.nombre,
          { tipo: "departamento" },
        ) if v.persona.departamento
        xml.observaciones(
          v.persona.municipio.nombre,
          { tipo: "municipio" },
        ) if v.persona.municipio
        xml.observaciones(
          v.persona.centropoblado.nombre,
          { tipo: "centro_poblado" },
        ) if v.persona.centropoblado
      end
    end
  end
  ...
  caso.victimacolectiva.each do |vc|
    xml.grupo do
      ...
      xml.observaciones(
        vc.grupoper.anotaciones,
        { tipo: "anotaciones" },
      ) if vc.grupoper
      xml.observaciones(
        vc.antecedente.map(&:nombre).join(";"),
        { tipo: "antecedente" },
      ) if vc.antecedente[0]
      xml.observaciones(
        vc.filiacion.map(&:nombre).join(";"),
        { tipo: "filiacion" },
      ) if vc.filiacion[0]
      xml.observaciones(
        vc.organizacion.map(&:nombre).join(";"),
        { tipo: "organizacion" },
      ) if vc.organizacion[0]
      xml.observaciones(
        vc.profesion.map(&:nombre).join(";"),
        { tipo: "profesion" },
      ) if vc.profesion[0]
      xml.observaciones(
        vc.rangoedad.map(&:nombre).join(";"),
        { tipo: "rangoedad" },
      ) if vc.rangoedad[0]
      xml.observaciones(
        vc.sectorsocial.map(&:nombre).join(";"),
        { tipo: "sectorsocial" },
      ) if vc.sectorsocial[0]
      xml.observaciones(
        vc.vinculoestado.map(&:nombre).join(";"),
        { tipo: "vinculoestado" },
      ) if vc.vinculoestado[0]
      xml.observaciones(
        vc.etnia.map(&:nombre).join(";"),
        { tipo: "etnia" },
      ) if vc.etnia[0]
    end
  end
  ...
  if caso.victima
    caso.victima.each do |v|
      xml.victima do
        ...
        xml.observaciones(
          v.contextovictima.map(&:nombre).join(";"),
          { tipo: "contexto" },
        ) if v.contextovictima[0]
        xml.observaciones(
          v.otraorga.map(&:nombre).join(";"),
          { tipo: "otraorga" },
        ) if v.otraorga[0]
        xml.observaciones(
          v.profesionsec.map(&:nombre).join(";"),
          { tipo: "profesionsec" },
        ) if v.profesionsec[0]
        xml.observaciones(
          v.sectorsocialsec.map(&:nombre).join(";"),
          { tipo: "sectorsocialsec" },
        ) if v.sectorsocialsec[0]
        xml.observaciones(
          v.filiacion.nombre, { tipo: "filiacion" }
        ) if v.filiacion
        xml.observaciones(
          v.orientacionsexual,
        ...
        xml.observaciones(
          v.vinculoestado.nombre, { tipo: "vinculoestado" }
        ) if v.vinculoestado
        ...
        xml.observaciones(
          v.rangoedad.nombre, { tipo: "rangoedad" }
        ) if v.rangoedad
        xml.observaciones(
          v.antecedente.map(&:nombre).join(";"),
          { tipo: "antecedente" },
        ) if v.antecedente[0]
      end
    end
  ...
  if caso.combatiente
    caso.combatiente.each do |co|
      xml.combatiente do
        ...
        xml.observaciones(
          co.rangoedad.nombre.strip, { tipo: "rangoedad" }
        ) if co.rangoedad
        xml.observaciones(
          co.filiacion.nombre.strip, { tipo: "filiacion" }
        ) if co.filiacion
        xml.observaciones(
          co.vinculoestado.nombre.strip, { tipo: "vinculo_estado" }
        ) if co.vinculoestado
  ...
  xml.observaciones(
    ...
    { tipo: "intervalo" },
  ) if caso.intervalo && caso.intervalo.nombre
  xml.observaciones(
    caso.region.map(&:nombre).join("; "), { tipo: "region" }
  ) if caso.region
  xml.observaciones(
