  ...
  module VistaHelper
    ...
    def embeber_mapacalordepcol_svg(titulo, subtitulo, titulorangos, fuente,
      ...
      if opciones[:colormax_r].present? && opciones[:colormax_v].present? &&
        ...
        if cr.between?(0, 255) && cv >= 0 && cv <= 255 && ca <= 0 && ca <= 255
          cr = crt
          cv = cvt
          ca = cat
        end
      ...
      cantidadesdep.each do |iddep, v|
        next unless Msip::Departamento.where(
    ...
    def embeber_mapacalormuncol_svg(
      ...
      cantidadesmun.each do |idmun, v|
        next unless !idmun || Msip::Municipio.where(
