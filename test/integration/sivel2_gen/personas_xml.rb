...
module Sivel2Gen
  class PersonasXml < ActionDispatch::IntegrationTest
   ...
   test "valida caso con 2 victima colectivas" do
        ...
        victimacolectiva1= Sivel2Gen::Victimacolectiva.create(id_grupoper: grupoper1.id, id_caso: caso.id, personasaprox: 5, organizacionarmada:5)
        victimacolectiva2= Sivel2Gen::Victimacolectiva.create(id_grupoper: grupoper2.id, id_caso: caso.id, personasaprox: 5, organizacionarmada:5)
         get caso_path(caso)+".xml"
         #puts @response.body
         file= guarda_xml(@response.body)	
         docu = File.read(file)
