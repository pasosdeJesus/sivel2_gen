...
CREATE VIEW public.cben1 AS
 SELECT caso.id AS id_caso,
  ...
  WHERE ((caso.fecha >= '2021-01-01'::date) AND (caso.fecha <= '2021-12-31'::date) AND (subv.id_victima = victima.id) AND (caso.id = victima.id_caso));
...
CREATE VIEW public.cben2 AS
 SELECT cben1.id_caso,
  ...
  GROUP BY cben1.id_caso, cben1.id_victima, cben1.id_persona, cben1.npersona, cben1.id_sectorsocial, ubicacion.id_departamento, departamento.nombre, ubicacion.id_municipio, municipio.nombre, ubicacion.id_clase, clase.nombre;
...
CREATE VIEW public.cvt1 AS
 SELECT DISTINCT acto.id_caso,
    ...
    categoria.nombre AS categoria,
        CASE
            WHEN (persona.sexo = 'F'::bpchar) THEN 'Femenino'::text
            WHEN (persona.sexo = 'M'::bpchar) THEN 'Masculino'::text
   ...
   FROM ((((((public.sivel2_gen_acto acto
     ...
     LEFT JOIN public.sivel2_gen_profesion profesion ON ((victima.id_profesion = profesion.id)))
  WHERE ((caso.fecha >= '2021-01-01'::date) AND (caso.fecha <= '2021-12-31'::date));
