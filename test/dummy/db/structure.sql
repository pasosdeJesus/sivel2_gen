...
--
-- Name: sip_municipio_sivel2_gen_region; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.sip_municipio_sivel2_gen_region (
    sivel2_gen_region_id bigint NOT NULL,
    sip_municipio_id bigint NOT NULL
);
 ...
 SELECT conscaso.caso_id,
  ...
  WHERE (conscaso.caso_id IN ( SELECT sivel2_gen_conscaso.caso_id
          ...
          WHERE ((sivel2_gen_conscaso.caso_id IN ( SELECT sivel2_gen_caso.id
                   FROM public.sivel2_gen_caso)) AND (sivel2_gen_conscaso.caso_id IN ( SELECT sivel2_gen_caso.id
                   FROM public.sivel2_gen_caso
                  WHERE (sivel2_gen_caso.created_at <= '2021-05-20 00:00:00'::timestamp without time zone))) AND (sivel2_gen_conscaso.q @@ plainto_tsquery('spanish'::regconfig, public.unaccent('2044'::text))))
